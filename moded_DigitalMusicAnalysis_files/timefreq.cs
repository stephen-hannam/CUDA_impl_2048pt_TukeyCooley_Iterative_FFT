...
namespace DigitalMusicAnalysis
{
    public class timefreq
    {
        ...
        public timefreq(float[] x, int windowSamp)
        {
            ...
            //string freqfilename = @"C:\CAB401data\timefreq_out";
            //FileStream freqoutFile = File.Create(freqfilename);
            //BinaryWriter freqbinWrite = new BinaryWriter(freqoutFile);
            ...
            //for (int ii = 0; ii < wSamp/2; ii++) {
            //    for (int jj = 0; jj < cols; jj++) {
            //        freqbinWrite.Write(timeFreqData[ii][jj]);
            //    }
            //}
            //freqbinWrite.Flush();
            //freqbinWrite.Close();
            //long length = new System.IO.FileInfo(freqfilename).Length;
            //Console.WriteLine("Write: length in elements {0}, length in bytes {1}",(cols*wSamp/2),length);
            
            //freqbinWrite.Dispose();
        }
        ...
        float[][] stft(Complex[] x, int wSamp)
        {
            ...
            //fftw_complexarray temp, tempFFT;
            //temp = new fftw_complexarray(wSamp);
            //tempFFT = new fftw_complexarray(wSamp);
            ...
            for (ii = 0; ii < 2 * Math.Floor((double)N / (double)wSamp) - 1; ii++)
            {
                ...
                tempFFT = fft(temp);
                /* use basic single-threaded FFTW equivalent instead */
            ...
            //Console.WriteLine("fftMax: {0}", fftMax);
